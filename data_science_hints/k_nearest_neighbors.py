...
def run_compare_neighbors_count():
    for k in [1, 3, 5, 7]:
        num_correct = 0
        ...
        for city in cities:
            ...
            if predicated_language == actual_language:
                num_correct += 1
        print("Num of neighbors:", k, num_correct, "correct of total", len(cities))

def run_plot_diagram_of_prediction():
    plots = {"Java": ([], []), "Python": ([], []), "R": ([], [])}
    markers = {"Java": "o", "Python": "s", "R": "^"}
    colors = {"Java": "r", "Python": "b", "R": "g"}
    k = 5
...
def run_dimention_dependency_diagram():
    ...
    plt.plot(dimensions, avg_distances, color='green', marker='.', linestyle='solid', label="avg")
    plt.plot(dimensions, min_distances, color='red', marker='.', linestyle='solid', label="min")
    plt.legend(loc=0)
    plt.title("AVG VS MIN (with raise dimension)")
    plt.grid()
    plt.show()

if __name__ == "__main__":
    run_plot_apriory()
    run_compare_neighbors_count()
    run_plot_diagram_of_prediction()
    run_dimention_dependency_diagram()